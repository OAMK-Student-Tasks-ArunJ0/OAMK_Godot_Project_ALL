...
func drop_items() -> void:
	...
	for i in drops.size():
		...
		for j in drop_count:
			...
			# Destroy state only happens when area2d sends a signal so i have to use call_deferred insteead of just "add_child"
			# basically call deferred makes sure it is safe to add a child
